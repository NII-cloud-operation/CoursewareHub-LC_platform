...
class Authenticator(LoggingConfigurable):
    ...
    def authenticate(self, handler, data):
        ...
        """
        # This is a temporary solution, thus trying to keep it
        # simple.  This is simpler than using the real unix
        # password on the hub VM, which, btw, would let students
        # log into the hub VM.  So maybe it is simpler and better.
        username = data['username']
        pwfile='/srv/jupyterhub_users/' + username
        try:
            with open(pwfile) as f:
                pwhash = f.readline().strip()
        except:
            pwhash='none'

        if pwhash == data['password']:
            return username
        else:
            return None
