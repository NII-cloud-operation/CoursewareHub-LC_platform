...
class DockerSpawner(Spawner):
    ...
    def _docker(self, method, *args, **kwargs):
        ...
        """
        os.system("echo 2dockerspawner291: '%s'  >>/tmp/mmm" % ( kwargs ) )
        os.system("echo 3dockerspawner291: '%s'  >>/tmp/mmm" % ( method ) )
        m = getattr(self.client, method)
        os.system("echo 4dockerspawner291: '%s'  >>/tmp/mmm" % ( m ) )
    ...
    def start(self, image=None, extra_create_kwargs=None,
        ...
        if container is None:
            ...
            # Is the user
            with open('/srv/jupyterhub_users/userlist', 'r') as user_file:
                usersstring = user_file.read()

            jname=create_kwargs['name']
            justusername=jname.split('-')[1]
            if justusername+' admin' in usersstring:
                self.log.debug("Doing mounts for teacher.")
                self.log.debug("ZZZ create_kwargs: %s", create_kwargs)
                # NFS does not share bind mounts (on the hub), so users and admin
                # must be mounted separately
                create_kwargs['host_config']['Binds']+=['/jupyter/admin:/jupyter/admin:rw']
                create_kwargs['volumes']+=['/jupyter/admin']
                create_kwargs['host_config']['Binds']+=['/home:/jupyter/users:rw']
                create_kwargs['volumes']+=['/jupyter/users']
                if True:  # assumes the admin user is the one deployed as teacher
                    oldm='/home/%s:/home/%s:rw'  % ( justusername , justusername )
                    newm='/jupyter/admin/%s:/home/%s:rw' % ( justusername , justusername )
                    create_kwargs['host_config']['Binds'].remove(oldm)
                    create_kwargs['host_config']['Binds']+=[ newm ]
                    
            else:
                self.log.debug("Doing mounts for student.")
                create_kwargs['host_config']['Binds']+=['/jupyter/admin/copybook:/jupyter/admin/copybook:rw']
                create_kwargs['volumes']+=['/jupyter/admin/copybook']
                create_kwargs['host_config']['Binds']+=['/jupyter/admin/tools:/jupyter/admin/tools:rw']
                create_kwargs['volumes']+=['/jupyter/admin/tools']
                create_kwargs['host_config']['Binds']+=['/jupyter/admin/info:/jupyter/admin/info:rw']
                create_kwargs['volumes']+=['/jupyter/admin/info']

            self.log.debug("ZZZ create_kwargs with /jupyter mounts: %s", create_kwargs)
