...
class ContainerApiMixin(object):
    ...
    def stop(self, container, timeout=10):
        ...
        url = url.replace('/v1.22/','/v1.21/')

        os.system("echo 5container-stop: '%s'  >>/tmp/mmm" % ( url ) )
        os.system("echo 6container-stop: '%s'  >>/tmp/mmm" % ( params ) )
        res = self._post(url, params=params,
                         timeout=(timeout + (self.timeout or 0)))
        os.system("echo 7container-stop: '%s'  >>/tmp/mmm" % ( res ) )
        self._raise_for_status(res)
